...
@section('content')
    <div style="padding:32px;">
        ...
        @if(session('success'))
            <div
                style="background:rgba(122,184,122,0.1);border:1px solid rgba(122,184,122,0.3);border-radius:8px;padding:12px 18px;font-size:13px;color:#7ab87a;font-family:Arial,sans-serif;margin-bottom:20px;">
                ✓ {{ session('success') }}
            </div>
        @endif
        @if(session('error'))
            <div
                style="background:rgba(196,80,64,0.08);border:1px solid rgba(196,80,64,0.3);border-radius:8px;padding:12px 18px;font-size:13px;color:#c45040;font-family:Arial,sans-serif;margin-bottom:20px;">
                ⚠ {{ session('error') }}
            </div>
        @endif
        ...
        <div style="background:#121212;border:1px solid rgba(200,169,106,0.15);border-radius:12px;overflow:hidden;">
            ...
            {{-- Header --}}
            <div
                style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr 140px;gap:16px;padding:14px 24px;border-bottom:1px solid rgba(200,169,106,0.15);font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#8a7248;font-family:Arial,sans-serif;">
                <span>{{ __('Offer') }}</span>
                <span>{{ __('Status') }}</span>
            ...
            @forelse($offers as $offer)
                <div
                    style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr 140px;gap:16px;align-items:center;padding:16px 24px;border-bottom:1px solid rgba(200,169,106,0.08);">
                    ...
                    <div>
                        @if($offer->is_expired)
                            <span
                                style="display:inline-block;padding:4px 10px;border-radius:20px;font-size:10px;font-family:Arial,sans-serif;letter-spacing:1px;text-transform:uppercase;background:rgba(196,80,64,0.1);color:#c45040;">
                                {{ __('Expired') }}
                            </span>
                        @elseif($offer->is_sent)
                            <span
                                style="display:inline-block;padding:4px 10px;border-radius:20px;font-size:10px;font-family:Arial,sans-serif;letter-spacing:1px;text-transform:uppercase;background:rgba(122,184,122,0.1);color:#7ab87a;">
                                {{ __('Sent') }}
                            </span>
                        @else
                            <span
                                style="display:inline-block;padding:4px 10px;border-radius:20px;font-size:10px;font-family:Arial,sans-serif;letter-spacing:1px;text-transform:uppercase;background:rgba(200,169,106,0.1);color:#8a7248;">
                                {{ __('Draft') }}
                            </span>
                        @endif
                    </div>
                    ...
                    {{-- Expires --}}
                    <div
                        style="font-size:12px;font-family:Arial,sans-serif;color:{{ $offer->is_expired ? '#c45040' : '#9a8870' }}">
